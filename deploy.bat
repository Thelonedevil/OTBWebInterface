...
echo ^<settings^>^<servers^>^<server^>^<id^>snapshots^</id^>^<username^>deployment^</username^>^<password^>%password%^</password^>^</server^>^<server^>^<id^>releases^</id^>^<username^>deployment^</username^>^<password^>%password%^</password^>^</server^>^</servers^>^</settings^> > settings.xml
